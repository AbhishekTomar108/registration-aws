...
const EditRunningBatch = () => {
    ...
    const getRunningBatchTrainer = async (trainer) => {
      ...
      try {
          let runningTrainer = await fetch('http://127.0.0.1:8000/getRunningBatchTrainer', {
              method: 'POST',
...
const updateBatch = async () => {
  ...
  ContextValue.updateProgress(30)
    ContextValue.updateBarStatus(true)
  let addedNewBatch = await fetch(`http://127.0.0.1:8000/updateBatch/${runningBatch._id}`, {
      method: 'POST',
      headers: { "Content-Type": "application/json" },
