...
const Assignment = (props) => {
  ...
  let contextValue = useContext(StudentContext);
    ...
    const sendAssignment = async (e,id,url,file) => {
        ...
        try {
            const res = await fetch('http://127.0.0.1:8000/submititem', {
                method: 'POST',
                body: formData,
            });
    ...
    const getTrainerdata = async () => {
        const res = await fetch("http://127.0.0.1:8000/getuploadAssignmentUrl", {
    ...
    const downloaduser = async (url) => {
        ...
        const res2 = await fetch(`http://127.0.0.1:8000/files/${url}`, {
            method: "GET",
            headers: {
                "Content-Type": "application/json"
            }
