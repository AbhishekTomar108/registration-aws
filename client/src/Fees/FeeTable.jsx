...
function FeeTable() {
  ...
  const getTrainerdata = async () => {
    const res = await fetch("http://127.0.0.1:8000/FeeTable", {
      method: "GET",
      headers: {
  ...
  const deleteuser = async (id) => {
    Swal.fire({
      ...
      if (result.isConfirmed) {
        fetch(`http://127.0.0.1:8000/deleteFee/${id}`, {
          method: "DELETE",
          headers: {
            "Content-Type": "application/json"
          }
  ...
  const getAllStudentPaymentStatus =async()=>{
    ...
    let studentPaymentStatus = await fetch("http://127.0.0.1:8000/getStudentFeesStatus")
  ...
  const SearchStudentFees = async()=>{
    ...
    let getRangeFees = await fetch("http://127.0.0.1:8000/getRangeFees",{
      method:"GET",
      headers:{
